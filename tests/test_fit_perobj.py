...
def test_s0():
    ...
    FitObj = SimFit(r, lnA_bounds=(np.log(A) - 5, np.log(A) + 5))#,weight_scale=0)
...
def test_weighted():
    ...
    FitObj = SimFit(10 ** bins[:-1][counts > 0], nm=counts[counts > 0])#,weight_scale=0)
...
def test_multisim():
    ...
    FitObj = SimFit(np.concatenate((r, r)))#,weight_scale=0)
    res1,po1 = FitObj.run_downhill(hs0=14, alpha0=-1.8, beta0=1.0)

    r = [r,r]
    FitObj = SimFit(r)#,weight_scale=0)
    res,po = FitObj.run_downhill(hs0=14, alpha0=-1.8, beta0=1.0)
