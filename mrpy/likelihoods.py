...
class PerObjLike(core.MRP):
    ...
    def __init__(self, logm, logHs, alpha, beta, lnA, log_mmin=None,
                 Om0=0.3, rhoc=2.7755e11):
        super(PerObjLike, self).__init__(logm, logHs, alpha, beta, lnA, log_mmin, Om0, rhoc)
    ...
    #     """
    #     return -self._q_/len(self.m) + self._scaled_mass*(self._lng)/self._mean_scaling
...
class CurveLike(PerObjLike):
    ...
    def __init__(self, logm, logHs, alpha, beta, lnA,
                 dndm=None, mw_data=None, mw_integ=None,
                 sig_data=1, sig_integ=np.inf, sig_rhomean=np.inf,
                 log_mmin=None, Om0=0.3, rhoc=2.7755e11):

        super(CurveLike, self).__init__(logm, logHs, alpha, beta, lnA, log_mmin, Om0, rhoc)
        ...
        if sig_integ == 0 and sig_rhomean == 0:
            ...
            # self.lnA = np.log(A)
            # self.alpha = self.alpha + self.scale

        elif sig_integ == 0:
            self._norm = np.log(self.mw_integ) - np.log(self._q_) - self.logHs*ln10
        elif sig_rhomean == 0:
    ...
    @_cached
    def _fdata(self):
        return self._lng + self.logHs*ln10 - np.log(self.mw_data)
    ...
    @_cached
    def _fint(self):
        return -np.log(self.mw_integ/self._q_) + self.logHs*ln10
    ...
    @_cached
    def _fdata_jac(self):
        return np.array([self._lng_h + ln10, self._lng_a, self._lng_b])
    ...
    @_cached
    def _fint_jac(self):
        return np.array([self._q_h_/self._q_ + ln10, self._q_a_/self._q_, self._q_b_/self._q_])
    ...
    def _delta_integ_hess(self):
        out = np.zeros((4, 4))
        out = self._q_hess_/self._q_ - self._q_jac_**2/self._q_**2
        return out
